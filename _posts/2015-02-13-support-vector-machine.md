...
$$ arg\hspace{2 pt} \max_{\mathbf{w}, b} \{\frac{1}{\|\mathbf{w}\|} \min_n [t_n (\mathbf{w}^T \phi (\mathbf{x})+b)] \} \tag{1} $$
...
$$ \tilde{L}(\mathbf{a})=\sum_{n=1}^Na_n=\frac{1}{2} \sum_{n=1}^N \sum_{m=1}^N a_n a_m t_n t_m k(\mathbf{x}_n, \mathbf{m}_m)\tag{4} $$
